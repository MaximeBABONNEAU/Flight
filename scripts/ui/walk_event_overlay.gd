...
func show_event(text: String, labels: Array[String]) -> void:
	...
	_auto_respond_timer = 0.0
	# Track time for choice_latency on click (consumed by WalkEventController).
	# Cycle 10 — card_shown telemetry moved to WalkEventController._trigger_event
	# where the full event Dict is available to compute has_risk_hint and
	# axis_diversity correctly (the overlay only sees text + labels[]).
